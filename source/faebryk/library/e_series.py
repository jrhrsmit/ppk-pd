...
def e_series_ratio(
    ...
    """
    Calculates the values for two components in the E series range which are bound by a ratio.
    ...
    Returns a tuple of R1/R2 values, of which R1 is decided by component_value, and R2 is chosen from the E series based on ratio.
    ...
    if type(R1) is Constant:
        R1_value = R1.get_trait(
            is_representable_by_single_value
        ).get_single_representing_value()

        if type(output_input_ratio) is Constant:
            output_input_ratio_value = output_input_ratio.get_trait(
                is_representable_by_single_value
            ).get_single_representing_value()
            # output voltage can't be the same or higher than the input voltage, and can't be 0
            assert output_input_ratio_value < 1 and output_input_ratio_value > 0
            ...
            assert (
                output_input_ratio.min < 1 and output_input_ratio.min > 0
            ), "output voltage can't be the same or higher than the input voltage, and can't be 0"
            assert (
                output_input_ratio.max < 1 and output_input_ratio.max > 0
            ), "output voltage can't be the same or higher than the input voltage, and can't be 0"
            ...
            assert (
                ...
                and real_ratio >= output_input_ratio.min
            ), "Calculated optimum R1 R2 value pair gives output/input voltage ratio outside of specified range. Consider Increasing the output/input range or using a broader E value series"
        ...
        if type(output_input_ratio) is Constant:
            ...
            assert (
                output_input_ratio_value < 1 and output_input_ratio_value > 0
            ), "output voltage can't be the same or higher than the input voltage, and can't be 0"
            ...
            if (
                ...
                raise ValueError(
                    "Calculated optimum R1 R2 value pair gives output/input voltage ratio outside of specified range. Consider Increasing the output/input range, R1 value range, or using a broader E value series"
                )
